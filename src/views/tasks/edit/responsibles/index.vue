	...
	<ion-page>
		<ion-header>
			<ion-toolbar>
				...
				<ion-title>{{ title !== "" ? title : "Ответственные" }}</ion-title>
			</ion-toolbar>
		</ion-header>
			...
			<section class="content">
				<ion-card v-for="task in tasks">
					<ion-card-content class="ion-no-padding">
						<ion-list lines="none">
							<ion-item v-if="task.responsibles.length === 0">Ответственные не указаны</ion-item>
							<ion-item v-else v-for="responsible in task.responsibles">
								<ion-label>
									<h2>Ответственный</h2>
									<p>{{ responsible.responsible }}</p>
								</ion-label>
								<ion-button slot="end" color="danger"
											@click="deleteResponsible(responsible.responsibleId)">
									Удалить
								</ion-button>
							</ion-item>
						</ion-list>
		...
		>
			<div class="block ion-padding">
				<div class="modal-content" v-if="!pending">
					<p>Ответственный снят с задачи</p>
...
const deleteResponsible = async (oId: string) => {
	let data = await TaskService.deleteResponsible(taskUID.value, oId);
	const index = tasks.value[0].responsibles.findIndex(
		(responsible) => responsible.responsibleId === oId,
	);
