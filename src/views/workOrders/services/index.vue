    ...
    <ion-page>
        <ion-header>
            <ion-toolbar>
                ...
                <ion-title>{{ title !== '' ? title : "Услуги" }}</ion-title>
            </ion-toolbar>
        </ion-header>
        ...
        <ion-content :fullscreen="true">
            ...
            <section class="content">
                <ion-title v-if="workOrderServices.length == 0">
                ...
                <ion-card v-for="workOrderService in workOrderServices" :key="workOrderService.rowId">
...
import { WorkOrderService } from "@/interfaces/workorderservice.interface";
...
const workOrderServices = ref<WorkOrderService[]>([]);
...
const fetchTaskData = async (scanCode: string) => {
    ...
    try {
        task_data = await HttpService.post({
            url: `${API_URL}/GetWorkOrderServicesById/v3/`,
            params: {
    ...
    if (task_data && task_data.data.length > 0) {
        workOrderServices.value = task_data.data[0];
...
const fetchWorkOrderProducts = async (workOrderId: string) => {
    ...
    if (task_data && task_data.data.length > 0) {
        workOrderServices.value = [];
        workOrderServices.value.push(...task_data.data)
        console.log("WorkOrderProducts:", workOrderServices.value);

        title.value = `Услуги заказ-наряда`
