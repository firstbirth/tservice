                    ...
                    <ion-list :inset="false">
                        ...
                        <ion-item :button="true" :detail="false">
                            <!-- <ion-label>
                                ...
                                <p>{{ product?.storagePlaces.length ? product.storagePlaces : 'Не указано' }}</p>
                            </ion-label> -->
                            <ion-label>
                                <h2>Места хранения</h2>
                                <p v-if="product?.storagePlaces.length">
                                <ul class="analogues-list">
                                    <li v-for="(places, index) in product.storagePlaces" :key="index">
...
const scancode = pathParts[pathParts.length - 1];
...
const fetchTaskData = async (scanCode: string) => {
    ...
    if (task_data && task_data.data.length > 0) {
        product.value = task_data.data[0];
        title.value = product.value?.productName
...
.analogues-list li {
    margin-bottom: 0.5rem; /* Расстояние между элементами */
}
.attachments-content {
